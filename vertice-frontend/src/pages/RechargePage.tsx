    ...
    const handleUpdateStatus = async (id: number, status: string) => {
        try {
            ...
            const msg = err.response?.data?.message ||
                (err.response?.status === 404 ? 'Error: El servidor no reconoce esta acción. Intente reiniciar el servidor backend.' : 'Error al actualizar estado');
            setError(msg);
        }
    };
...
<body>
  ...
  <div class="center">¡Gracias!</div>
    ...
    return (
        <Box sx={{ p: 3 }}>
            <Typography variant="h4" gutterBottom>Punto de Recarga</Typography>

            {error && <Alert severity="error" sx={{ mb: 2 }} onClose={() => setError(null)}>{error}</Alert>}

            <Paper sx={{ p: 3, mb: 3 }}>
                <Typography variant="h6" gutterBottom>Selecciona el servicio</Typography>
                <Grid container spacing={1} sx={{ mb: 3 }}>
                    {services.map((service) => {
                        const config = serviceConfig[service.name] || { icon: <PhoneAndroidIcon sx={{ fontSize: 28 }} />, isContract: false, color: '#888' };
                        ...
                        return (
                            <Grid item xs={4} sm={3} md={2} lg={1.7} key={service.id}>
                                <Card
                                    ...
                                    onClick={() => { setSelectedService(service); setPhoneNumber(''); }}
                                >
                                    <Box sx={{ color: isSelected ? config.color : 'text.secondary' }}>
                                        {config.icon}
                                    </Box>
                                    <Typography variant="caption" sx={{ textAlign: 'center', fontSize: '0.7rem', fontWeight: isSelected ? 'bold' : 'normal', color: isSelected ? config.color : 'text.primary' }}>
                                        {service.name}
                                    </Typography>
                                </Card>
                            </Grid>
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={6} sm={2}>
                        <TextField fullWidth size="small" label="Monto (Bs)" type="number" value={amountBs} onChange={(e) => setAmountBs(e.target.value)} />
                    </Grid>
                    <Grid item xs={6} sm={2}>
                        <TextField fullWidth size="small" label="Comisión %" type="number" value={commissionPercent} onChange={(e) => handleCommissionChange(e.target.value)} />
                    </Grid>
                    <Grid item xs={6} sm={2}>
                        <Button
                            fullWidth
                ...
                {amount > 0 && (
                    <Box sx={{ mt: 2, display: 'flex', gap: 3, justifyContent: 'flex-end' }}>
                        <Typography variant="body2">Monto: <strong>Bs. {amount.toFixed(2)}</strong></Typography>
                        <Typography variant="body2">Comisión: <strong>Bs. {commissionBs.toFixed(2)}</strong></Typography>
                        <Typography variant="body1" color="primary">Total: <strong>Bs. {totalChargeBs.toFixed(2)}</strong> (${totalChargeUsd.toFixed(2)})</Typography>
                    </Box>
                )}
            </Paper>
            ...
            <Paper sx={{ p: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                    ...
                    <Typography variant="body2">Total: <strong>Bs. {totalRechargesBs.toFixed(2)}</strong> (<strong>${totalRechargesUsd.toFixed(2)}</strong>)</Typography>
                </Box>
                            ...
                            {paginatedRecharges.length === 0 ? (
                                ...
                                paginatedRecharges.map((recharge) => (
                                    <TableRow key={recharge.id}>
                                        <TableCell>{new Date(recharge.createdAt).toLocaleTimeString('es-VE', { hour: '2-digit', minute: '2-digit' })}</TableCell>
                                        ...
                                        <TableCell align="right"><strong>Bs.{recharge.totalChargeBs.toFixed(2)}</strong></TableCell>
                                        <TableCell align="center">
                                            {recharge.status === 'RECHARGED' ? (
                                                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'success.main', gap: 0.5 }}>
                                                    ...
                                                    <Typography variant="caption" sx={{ fontWeight: 'bold' }}>RECARGADO</Typography>
                                                </Box>
                                            ) : (
                                                <Box sx={{ color: 'warning.main' }}>
                                                    <Typography variant="caption" sx={{ fontWeight: 'bold' }}>PENDIENTE</Typography>
                                                </Box>
                                            )}
                                        </TableCell>
                                        <TableCell>
                                            <Box sx={{ display: 'flex', gap: 0.5 }}>
                                                {recharge.status === 'PENDING' && (
                                                    <Tooltip title="Marcar como Recargado">
                                                        <IconButton size="small" color="success" onClick={() => handleUpdateStatus(recharge.id, 'RECHARGED')}>
            ...
            <Snackbar open={success} autoHideDuration={3000} onClose={() => setSuccess(false)} message="Recarga registrada exitosamente" />
            <Snackbar open={cartSuccess} autoHideDuration={2000} onClose={() => setCartSuccess(false)} message="Recarga agregada al carrito" />
        </Box>
    );
};
