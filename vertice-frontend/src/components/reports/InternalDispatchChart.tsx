...
const InternalDispatchChart = ({ data }: InternalDispatchChartProps) => {
  if (!data || data.length === 0) {
    return (
      <Box
        sx={{ p: 3, textAlign: 'center', height: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      >
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
        ...
        <XAxis dataKey="fecha" stroke="#808e9b" tick={{ fill: '#808e9b' }} tickLine={false} />
